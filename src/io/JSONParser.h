...
enum RSJresourceType { RSJ_UNINITIATED,
                       RSJ_UNKNOWN,
                       RSJ_OBJECT,
                       RSJ_ARRAY,
                       RSJ_LEAF };
...
inline std::string to_string(RSJresourceType rt) {
    switch (rt) {
        case RSJ_UNINITIATED:
            return ("RSJ_UNINITIATED");
        case RSJ_UNKNOWN:
            return ("RSJ_UNKNOWN");
        case RSJ_OBJECT:
            return ("RSJ_OBJECT");
...
inline std::string strip_outer_quotes(std::string str, char* qq = NULL) {
    str = strtrim(str);
...
inline int is_bracket(char c, std::vector<char const*>& bracks, int indx = 0) {
...
inline std::vector<std::string> split_RSJ_array(const std::string& str) {  // TODO: Make efficient. This function is speed bottleneck.
...
inline std::string insert_tab_after_newlines(std::string str) {
    for (int a = 0; a < str.length(); ++a)
        if (str[a] == '\n') {
...
class RSJresource {
   ...
   public:
    ...
    void print(bool print_comments = false, bool update_data = true) { std::cout << as_str(print_comments, update_data) << std::endl; }
...
inline int seek_next(std::string* str_p, int start_pos, char character) {
    return 0;
}
...
// RSJobject
template <>
inline RSJobject RSJresource::as<RSJobject>(const RSJobject& def) {  // returns copy -- for being consistent with other 'as' specializations
    if (!exists()) return (def);
    return (as_object());
}
...
// RSJarray
template <>
inline RSJarray RSJresource::as<RSJarray>(const RSJarray& def) {  // returns copy -- for being consistent with other 'as' specializations
    if (!exists()) return (def);
    return (as_array());
}
...
// String
template <>
inline std::string RSJresource::as<std::string>(const std::string& def) {
    if (!exists()) return (def);
...
// integer
template <>
inline int RSJresource::as<int>(const int& def) {
    if (!exists()) return (def);
...
// double
template <>
inline double RSJresource::as<double>(const double& def) {
    if (!exists()) return (def);
...
// bool
template <>
inline bool RSJresource::as<bool>(const bool& def) {
    if (!exists()) return (def);
